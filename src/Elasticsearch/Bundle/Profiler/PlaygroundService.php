...
namespace Elasticsearch\Bundle\Profiler;
...
final class PlaygroundService
{
    ...
    public function execute(string $query, ?string $index = null, string $operation = 'search'): array
    {
        ...
        $request = ['body' => $parsed['rawQuery']];
        $request['index'] = $this->normalizeIndex($index);

        if ('count' === $operation) {
            $response = $this->connection->getClient()->count($request);
        } else {
            $response = $this->connection->getClient()->search($request);
        }
    ...
    {
        $index = null !== $index ? trim($index) : null;
        if (null === $index || '' === $index) {
            throw new \InvalidArgumentException('Index is required.');
        }

        if (1 !== preg_match('/^[a-zA-Z0-9*._,-]+$/', $index)) {
     ...
     * @return string[]
     */
    private function getAllowedIndices(): array
    {
        $indices = [];
